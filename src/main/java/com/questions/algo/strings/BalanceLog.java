...
public class BalanceLog {
    ...
    public double balancePoint(Log log) {
        ...
        double totalW = log.weightUpto(log.length());
        totalW = Math.round(totalW*1000d);
        double halfW = totalW/2;
        while (start < end) {
            double wUpTo = log.weightUpto(start);
            wUpTo = Math.round(wUpTo*1000d);
            if (halfW <= wUpTo) break;
            start++;
        }
        return start;
    }
